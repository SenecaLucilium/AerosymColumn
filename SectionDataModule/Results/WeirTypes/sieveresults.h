...
class AdditionalSieveResults : public QDialog
{
    Q_OBJECT
public:
    explicit AdditionalSieveResults(QWidget *parent = nullptr)
        : QDialog(parent)
    {
...
public:
        ...
        : QWidget{parent}
    {
        ...
        {
            verificationResultsLabel = new QLabel("Поверочные проверки", verificationResultsContainer);
    ...
    void updateInformation(std::vector<std::shared_ptr<CalculationResults>> results)
    {
        for (auto& _ : results)
        {
            std::shared_ptr<SieveCalculationResults> weirRes = std::static_pointer_cast<SieveCalculationResults>(_);
            ...
            connect(additionalResultsButton, &QPushButton::clicked, this, [this, weirRes](){
                AdditionalSieveResults* additionalWidget = new AdditionalSieveResults(this);
                additionalWidget->updateInformation(weirRes);
                additionalWidget->show();
            });

            if (auto weirVerif = std::dynamic_pointer_cast<SieveCalculationResultsVerification>(weirRes)) {
                verificationResultsTable->insertColumn(newColumnIndex);
                verificationResultsTable->setHorizontalHeaderItem(newColumnIndex, new QTableWidgetItem(weirVerif->weirName));

                setTableItem(verificationResultsTable, 0, weirVerif->verification_columnDiameter);
                setTableItem(verificationResultsTable, 1, weirVerif->verification_perimeter);
                setTableItem(verificationResultsTable, 2, weirVerif->verification_crossSection);
                setTableItem(verificationResultsTable, 3, weirVerif->verification_workingPlateArea);
                setTableItem(verificationResultsTable, 4, weirVerif->verification_relativeColumnSection);
            }
        }
    }
